...
export const postApplication = catchAsyncError(async(req,res,next)=>{
    ...
    if(!allowedFormats.includes(resume.mimetype)){
        return next(new ErrorHandler("Envalid file type, Please upload in png, jpg or webp",400));
    }
    ...
    if(!cloudinaryResponse || cloudinaryResponse.error){
        ...
        return next(new ErrorHandler("failed to upload resume to Cloudnary",500));
    }
...
export const jobseekerGetAllApplication = catchAsyncError(async(req,res,next)=>{
    const {role} = req.user;
    if(role === "Employer"){
        return next(new ErrorHandler("Emplopyer is not allowed to access this resource!",400));
    } 
    const {_id} = req.user;
